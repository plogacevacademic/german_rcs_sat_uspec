...
```{DataAnalysisSummarize, echo=FALSE, results='hide', eval=TRUE}
...
data_rc_bysubj_dprime <- data_rc %>% subset(maxlik_interval) %>%
                        group_by(subject, gender, condition, interval) %>% 
                        summarize_n_responses() %>%
...
fit_brms <- readRDS("../workspace/model_fits/fit_all_uncorr_bysubj_1.rds")
