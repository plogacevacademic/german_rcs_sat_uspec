...
```{r illustration_uspec_smcm, results='hide'}
suppressWarnings({
source("../scripts_paper/illustrations_process_uspec_smcm.R")
})
```
...
![(\#fig:illustrationSMCMSteps)Illustration of the logic of the SMCM. Only one attachment process can succeed in unambiguous conditions (middle and lower panel), while the fastest attachment determines the duration of](extFigures/illustrationFlowSMCM.pdf)
...
![(\#fig:illustrationSMCMTime)Illustration of the logic of the SMCM. Only one attachment process can succeed in unambiguous conditions (middle and lower panel), while the fastest attachment process .](manualFigures/illustrationDurationsSMCM.pdf)
...
![(\#fig:illustrationEarlyUspec)The parser's actions when task demands allow underspecification:](extFigures/illustrationFlowEarlyUspec.pdf)
...
![(\#fig:illustrationLateUspec)The parser's actions when task demands allow underspecification:](extFigures/illustrationFlowLateUspec.pdf)
...
```{r illustrationSATFgenerate, results='hide'}
suppressWarnings({
source("../scripts_paper/illustrations_satf.R")
})
```

![(\#fig:illustrationSATF)A typical speed-accuracy tradeoff function. After an initial period of chance performance, accuracy begins to increase at the intercept ($\delta$). The growth rate $\beta$ determines how quickly function approaches asymptotic performance ($\lambda$). The reciprocal of the rate $\beta^{-1}$ can be interpeted as the time required for the function to reach approximately $63\%$ of the asymptote in time.](manualFigures/illustrationSATF.pdf){width="75%"}
...
![(\#fig:illustrationSATTrial)The structure of a SAT-trial.](extFigures/SATIllustration.pdf){width="25%"}
...
```{r illustrationSATFdiffs, results='hide', fig.height=1.5, fig.width=6, fig.cap="Hypothetical differences in speed-accuracy tradeoff functions."}
...
```{r illustration_predictions_uspec_smcm, results='hide'}

time <- seq(325,1100,1)
source("../scripts_paper/illustrations_uspec_smcm.R")
...
```{r predictionsUspec, results='hide', fig.height=2.5, fig.width=3, fig.cap="Probability of having successfully processed an RC attachment at a particular time as predicted by the underspecification account."}
# library(ggpubr)
# p <- ggarrange(illustration_cdf_uspec, illustration_cdf_smcm)
suppressWarnings({
print(prediction_cdf_uspec)
})
```
...
\end{equation}
```
```{r predictionsSMCM, results='hide', fig.height=2.5, fig.width=3, fig.cap="Probability of having successfully processed an RC attachment at a particular time as predicted by the SMCM."}
suppressWarnings({
print(prediction_cdf_smcm)
})
```
